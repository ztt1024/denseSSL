...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    _, ari, fscore, adjacc = eval_pred(real_labels, pred_labels, calc_acc=False)
    nmi = 0.
...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, student_local_cls, student_mask, epoch, in12=None, in21=None):
        ...
        dc_flag = False
        if student_mask[0].max() == 2:
            dc_flag = True
            dc_ls = []
            for id in range(len(student_mask)):
                dc_ind = (student_mask[id]==2).sum(dim=(1,2))>0
                dc_ls.append(dc_ind)
                student_mask[id] = student_mask[id]>0
                student_mask[id][dc_ind] = ~student_mask[id][dc_ind]

        B = student_mask[0].shape[0]
