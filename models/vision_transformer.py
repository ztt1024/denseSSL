...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(B, int(math.sqrt(N)), int(math.sqrt(N)), dim).permute(0, 3, 1, 2),
            scale_factor=(w0 / math.sqrt(N), h0 / math.sqrt(N)),
            mode='bicubic',
        )
        assert int(w0) == patch_pos_embed.shape[-2] and int(h0) == patch_pos_embed.shape[-1]
        patch_pos_embed = patch_pos_embed.permute(0, 2, 3, 1).view(B, -1, dim)
    ...
    def prepare_tokens(self, x, mask=None):
        ...
        # mask image modeling
        if mask is not None and mask.max() == 2:
            mask_dc = mask==2
            mask[mask==2] = 0
            x.permute(0, 2, 3, 1)[mask_dc, :] = x.flip(0).permute(0, 2, 3, 1)[mask_dc, :].detach()
